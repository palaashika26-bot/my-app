...
import { ArrowLeft, Download, AlertTriangle, MapPin, CheckCircle2, Circle, FileText, Info, Camera, X, ChevronLeft, ChevronRight, ZoomIn, MessageCircle, Upload, Package, Truck, Home } from 'lucide-react';
...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [concernImages, setConcernImages] = useState<File[]>([]);
  ...
  function handleConcernImageAdd(e: React.ChangeEvent<HTMLInputElement>) {
    if (e.target.files) {
      setConcernImages(prev => [...prev, ...Array.from(e.target.files!)]);
    }
  }
            ...
            <div className="p-5">
              {concernSubmitted ? (
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-600 text-foreground mb-1.5 block">Attach images <span className="text-muted-foreground font-400">(optional)</span></label>
                    <button
                      onClick={() => concernFileRef.current?.click()}
                      className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-dashed border-border hover:border-accent hover:bg-accent/5 text-sm text-muted-foreground hover:text-accent transition-colors w-full justify-center"
                    >
                      <Upload className="w-4 h-4" /> Click to upload images
                    </button>
                    <input
                      ref={concernFileRef}
                      type="file"
                    ...
                    {concernImages.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-2">
                        {concernImages.map((f, i) => (
                          <div key={i} className="flex items-center gap-1.5 px-2.5 py-1 bg-muted rounded-lg text-xs text-foreground">
                            <Package className="w-3 h-3 text-muted-foreground" />
                            {f.name}
                            <button onClick={() => setConcernImages(prev => prev.filter((_, j) => j !== i))} className="text-muted-foreground hover:text-red-500 ml-0.5"><X className="w-3 h-3" /></button>
                          </div>
                        ))}
                      </div>
                    )}
