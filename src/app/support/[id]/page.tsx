...
function Attachment({ url, onZoom }: { url: string; onZoom: (u: string) => void }) {
  ...
  if (isImage) return <img src={url} alt="attachment" onClick={() => onZoom(url)} className="w-20 h-20 object-cover rounded-lg border border-border cursor-zoom-in" />;
  if (isVideo) return <video src={url} controls className="w-32 h-20 rounded-lg border border-border bg-black" />;
...
export default function ClientTicketDetailPage() {
  ...
  async function send() {
    ...
    setSending(true);
    try {
      await supportApi.addMessage(id, { text: reply.trim(), attachments: files.map(f => f.base64) });
      setReply('');
      setFiles([]);
      await load();
    } catch {
