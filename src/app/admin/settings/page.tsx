  ...
  return (
    <AdminLayout>
      ...
      <div className="flex gap-1 mb-5 overflow-x-auto scrollbar-hide">
        {tabs.map(t => (<button key={t.id} onClick={() => setTab(t.id)} className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-600 whitespace-nowrap ${tab === t.id ? 'bg-[#5c5470] text-white' : 'text-muted-foreground hover:bg-muted'}`}><t.icon className="w-4 h-4" />{t.label}</button>))}
      </div>
        ...
        {tab === 'notif' && (
          <div className="max-w-3xl">
            ...
            <div className="space-y-3">{templates.map(t => (
              <div key={t.name} className="border border-border rounded-lg p-3"><div className="flex items-center justify-between"><p className="font-600 text-sm">{t.name}</p><button onClick={() => addToast({ type: 'info', title: 'Template editor opened' })} className="text-xs text-[#4A3B52] font-600 hover:underline">Edit</button></div><p className="text-xs text-muted-foreground mt-1">Subject: <span className="font-tabular">{t.subject}</span></p><p className="text-xs text-muted-foreground mt-0.5">{t.preview}</p></div>
            ))}</div>
          </div>
        )}
        {tab === 'admins' && (
          <div>
            ...
            <div className="bg-muted/30 rounded-xl overflow-hidden"><table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">{admins.map(a => <tr key={a.id} className="bg-card"><td className="px-3 py-2 font-500">{a.name}</td><td className="px-3 py-2 text-xs">{a.email}</td><td className="px-3 py-2"><span className="badge bg-[#4A3B52]/10 text-[#4A3B52]">{a.role}</span></td><td className="px-3 py-2 text-right">{a.role !== 'Super Admin' && <button onClick={() => removeAdmin(a.id)} className="text-xs text-red-500 hover:underline">Remove</button>}</td></tr>)}</tbody>
            </table></div>
            {showInvite && (
              <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center overflow-y-auto pt-4 md:pt-8" onClick={() => setShowInvite(false)}><div onClick={e => e.stopPropagation()} className="bg-card rounded-2xl w-full max-w-sm p-5 mb-4 mx-4">
