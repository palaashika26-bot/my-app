...
export default function AdminSuppliersPage() {
  ...
  return (
    <AdminLayout>
          ...
          <tbody className="divide-y divide-border">
            {filtered.map(s => (
              <tr key={s.id} className="table-row-hover">
                ...
                <td className="px-3 py-3 text-right"><div className="flex items-center justify-end gap-1"><button onClick={() => setViewing(s)} className="text-[11px] font-600 text-[#4A3B52] hover:underline">View</button><button onClick={() => remove(s.id)} className="p-1.5 rounded-md hover:bg-red-50 text-red-500"><Trash2 className="w-3.5 h-3.5" /></button></div></td>
              </tr>
            ))}
          </tbody>
        </table></div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center overflow-y-auto pt-4 md:pt-8" onClick={() => setShowAdd(false)}><div onClick={e => e.stopPropagation()} className="bg-card rounded-2xl w-full max-w-md p-5 mb-4 mx-4">
      ...
      {viewing && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-start justify-center overflow-y-auto pt-4 md:pt-8" onClick={() => setViewing(null)}><div onClick={e => e.stopPropagation()} className="bg-card rounded-2xl w-full max-w-lg p-5 mb-4 mx-4">
          ...
          <div><p className="text-[10px] uppercase text-muted-foreground mb-2">Product Catalog</p>
            <div className="grid grid-cols-2 gap-2">{Array.from({length:4}).map((_,i) => <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-muted/40"><div className="w-9 h-9 rounded-md bg-gradient-to-br from-[#E8E1F5] to-[#D6CEE8] flex items-center justify-center">📦</div><div className="text-xs"><p className="font-500">Sample Product {i+1}</p><p className="text-muted-foreground">¥10–20 • MOQ 100</p></div></div>)}</div>
          </div>
        </div></div>
      )}
    </AdminLayout>
