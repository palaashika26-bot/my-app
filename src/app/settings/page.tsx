...
function Toggle({ on, onChange, label }: { on: boolean; onChange: () => void; label: string }) {
  return (<label className="flex items-center justify-between py-3 border-b border-border last:border-0 cursor-pointer"><span className="text-sm">{label}</span><button onClick={onChange} className={`relative w-11 h-6 rounded-full transition-colors ${on ? 'bg-[#5c5470]' : 'bg-muted'}`}><span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${on ? 'translate-x-5' : ''}`} /></button></label>);
}
  ...
  return (
    <ClientLayout>
      ...
      <div className="flex gap-1 mb-5 overflow-x-auto scrollbar-hide">
        {tabs.map(t => (<button key={t.id} onClick={() => setTab(t.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-600 whitespace-nowrap ${tab === t.id ? 'bg-[#5c5470] text-white' : 'text-muted-foreground hover:bg-muted'}`}><t.icon className="w-4 h-4" />{t.label}</button>))}
      </div>
      <div className="bg-card rounded-xl border border-border shadow-card p-5">
        ...
        {tab === 'danger' && (<div><h3 className="font-700 text-red-600 mb-2">Danger Zone</h3><p className="text-sm text-muted-foreground mb-3">Deleting your account is permanent.</p><button onClick={() => setConfirm(true)} className="px-4 py-2 rounded-lg bg-red-500 text-white text-sm font-600 hover:bg-red-600">Delete Account</button>{confirm && <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center overflow-y-auto pt-4 md:pt-8" onClick={() => setConfirm(false)}><div onClick={e => e.stopPropagation()} className="bg-card p-6 rounded-2xl w-full max-w-sm mb-4 mx-4"><h4 className="font-700 mb-2">Are you sure?</h4><p className="text-sm text-muted-foreground mb-4">This action cannot be undone.</p><div className="flex gap-2"><button onClick={() => setConfirm(false)} className="btn-secondary flex-1 py-2 text-sm">Cancel</button><button onClick={() => { setConfirm(false); addToast({ type: 'info', title: 'Demo only', description: 'Account deletion is disabled.' }); }} className="flex-1 py-2 rounded-lg bg-red-500 text-white text-sm font-600">Delete</button></div></div></div>}</div>)}
      </div>
    </ClientLayout>
  );
}
