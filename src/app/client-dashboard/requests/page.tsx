  ...
  return (
    <ClientShell>
      ...
      <div className="flex gap-1 overflow-x-auto mb-4 scrollbar-hide">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-600 whitespace-nowrap transition-colors ${tab === t ? 'bg-accent text-white' : 'text-muted-foreground hover:bg-muted'}`}>{t}</button>
        ))}
      </div>
            ...
            <tbody className="divide-y divide-border">
              {filtered.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-muted-foreground">No requests in this filter.</td></tr> : filtered.map(r => (
                <tr key={r.id} className="table-row-hover">
                  <td className="px-4 py-3.5"><div className="flex items-center gap-2">{r.source === 'photo_scan' && <Camera className="w-3.5 h-3.5 text-accent" />}<span className="text-sm font-600 text-primary font-tabular">{r.requestId}</span></div></td>
                  ...
                  <td className="px-4 py-3.5"><Link href={`/client-dashboard/requests/${r.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-500 text-accent border border-accent/30 rounded-lg hover:bg-accent/10"><Eye className="w-3.5 h-3.5" /> View</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
