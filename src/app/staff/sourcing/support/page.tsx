...
export default function SourcingSupportPage() {
  ...
  async function handleFiles(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of picked) {
      if (file.size > MAX_BYTES) { addToast({ type: 'warning', title: `"${file.name}" exceeds 10 MB` }); continue; }
  ...
  function sendReply() {
    ...
    const msg: TicketMessage = {
      id: msgId, role: 'admin', text: reply.trim(),
      ...
      <div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-card rounded-xl border border-border shadow-card p-5">
                      ...
                      <div key={msg.id} className={`flex flex-col ${isAdmin ? 'items-end' : 'items-start'}`}>
                        <div className={`max-w-[80%] px-3 py-2 rounded-xl text-xs ${isAdmin ? 'bg-[#4A3B52] text-white' : 'bg-white border border-border text-foreground'}`}>
                          <p className={`text-[10px] font-600 mb-0.5 ${isAdmin ? 'text-white/80' : 'text-muted-foreground'}`}>
                            {isAdmin ? 'You (Staff)' : active.clientName}
                          </p>
                          {msg.text && <p>{msg.text}</p>}
                          {msg.attachments?.map((a, i) => (
                            <div key={i} className="mt-1">
                              {a.type === 'image' && <button onClick={() => setLightbox(a.base64)}><img src={a.base64} alt={a.name} className="max-w-[160px] max-h-[100px] rounded-lg object-cover" /></button>}
                              {a.type === 'pdf' && <div className="flex items-center gap-1 text-[10px]"><FileText className="w-3 h-3" /><a href={a.base64} download={a.name} className="underline">{a.name}</a></div>}
                            </div>
                          ))}
                        </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={pending.length >= MAX_FILES} className="p-2 text-muted-foreground hover:text-foreground rounded-lg hover:bg-muted/40 disabled:opacity-40 border border-border">
        ...
        {lightbox && (
          <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setLightbox(null)}>
            ...
            <button onClick={() => setLightbox(null)} className="absolute top-4 right-4 text-white bg-black/50 rounded-full p-2"><X className="w-5 h-5" /></button>
          </div>
        )}
      </div>
    );
                  ...
                  <tr key={t.id} onClick={() => setActive(t)} className="table-row-hover cursor-pointer">
                    <td className="px-3 py-3 font-tabular font-600 text-primary">
                      <div className="flex items-center gap-1.5">
                        {t.id}
                        {unread && <span className="w-2 h-2 rounded-full bg-red-500 inline-block" />}
                      </div>
                    </td>
                    <td className="px-3 py-3">
                      <p className="text-sm font-500">{t.clientName}</p>
                      <p className="text-[11px] text-muted-foreground">{t.clientEmail}</p>
                    </td>
