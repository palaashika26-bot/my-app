...
export default function SourcingLogisticsPage() {
  ...
  const active = shipments.length + requests.length;
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{active} active shipments in the pipeline</p>
        </div>
        <div className="flex gap-1">
          {['All', 'Sea Freight', 'Air Freight', 'Express'].map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 rounded-lg text-xs font-600 ${filter === f ? 'bg-[#4A3B52] text-white' : 'bg-muted text-muted-foreground hover:bg-muted/70'}`}>{f}</button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[900px]">
            <thead className="bg-muted/40 border-b border-border">
              <tr className="text-[11px] uppercase text-muted-foreground">
                <th className="px-3 py-3 text-left font-600">Tracking ID</th>
                <th className="px-3 py-3 text-left font-600">Order ID</th>
                <th className="px-3 py-3 text-left font-600">Client</th>
                <th className="px-3 py-3 text-left font-600">Carrier</th>
                <th className="px-3 py-3 text-left font-600">Current Location</th>
            ...
            <tbody className="divide-y divide-border">
              {mockFiltered.map(s => {
                const c = carrierForOrder(s.orderId);
                const loc = statusToLocation[s.status as string];
                return (
                  <tr key={s.id} className="table-row-hover">
                    <td className="px-3 py-3 font-tabular font-600">{c.trackingNo}</td>
                    <td className="px-3 py-3">
                      <Link href={`/staff/sourcing/orders/${s.id}`} className="font-tabular text-primary font-600 hover:text-[#4A3B52]">{s.orderId}</Link>
                    </td>
                    <td className="px-3 py-3 text-sm">{s.client}</td>
                    <td className="px-3 py-3">
                      <p className="text-sm">{c.carrier}</p>
                      <p className="text-[11px] text-muted-foreground">{c.mode}</p>
                    </td>
                    <td className="px-3 py-3">
                      <p className="text-sm">{loc.label}</p>
                      <div className="w-24 h-1 mt-1 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-accent to-[#1A1423]" style={{ width: `${loc.progress}%` }} />
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => addToast({ type: 'success', title: 'Location updated' })} className="p-1.5 rounded-md hover:bg-muted" title="Update">
                          <RefreshCw className="w-3.5 h-3.5" />
                        </button>
                        <Link href={`/staff/sourcing/orders/${s.id}`} className="p-1.5 rounded-md hover:bg-muted text-[#4A3B52]">
