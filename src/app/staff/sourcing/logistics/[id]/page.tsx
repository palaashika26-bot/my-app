...
export default function SourcingLogisticsDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [quoteForm, setQuoteForm] = useState<AdminQuote>({ carrier: '', shippingMode: 'Sea', estimatedPrice: '', pricePerKg: '', eta: '', note: '' });
  ...
  useEffect(() => {
    ...
    const slip = localStorage.getItem(`logistics-warehouse-slip-${id}`);
    setWarehouseSlip(slip);

    const cargRaw = localStorage.getItem(`logistics-cargo-confirmed-${id}`);
    if (cargRaw) setCargoConfirmation(JSON.parse(cargRaw));
    ...
    if (stored) {
      ...
      const seed: ChatMessage[] = [{ id: 'seed-1', sender: 'admin', text: 'We have received your logistics request and are reviewing the details. We\'ll send you a quote shortly.', time: 'Just now' }];
  ...
  async function handleConfirmCargo() {
    ...
    try {
      const confirmData: CargoConfirmation = { confirmedBy: staffName.trim(), confirmedAt: new Date().toISOString(), notes: staffNotes.trim(), requestId: id };
      localStorage.setItem(`logistics-cargo-confirmed-${id}`, JSON.stringify(confirmData));
      const all = loadRequests();
      const idx = all.findIndex(r => r.id === id);
      if (idx !== -1) { all[idx].status = 'CargoReceived'; all[idx].cargoConfirmed = true; saveRequests(all); setReq(all[idx]); }
  ...
  const clientInitials = req.clientName.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2);
            ...
            {POST_APPROVAL_STATUSES.includes(req.status) && req.adminQuote ? (
              <div>
                ...
                <div className="space-y-2 text-sm">
                  {[
                    ['Carrier', req.adminQuote.carrier], ['Shipping Mode', req.adminQuote.shippingMode],
                    ['Estimated Price', `₹${req.adminQuote.estimatedPrice}`], ['Price per KG', `¥${req.adminQuote.pricePerKg}`],
                  ...
                  {req.adminQuote.note && <p className="text-xs text-muted-foreground italic pt-1">{req.adminQuote.note}</p>}
                </div>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input className="input-field text-sm" placeholder="e.g. COSCO, DHL, FedEx" value={quoteForm.carrier} onChange={e => { setQuoteForm(f => ({ ...f, carrier: e.target.value })); setQuoteSaved(false); }} />
                  </div>
                  <div>
                    <label className="text-xs font-600 block mb-1">Shipping Mode</label>
                    <select className="input-field text-sm" value={quoteForm.shippingMode} onChange={e => { setQuoteForm(f => ({ ...f, shippingMode: e.target.value })); setQuoteSaved(false); }}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input className="input-field text-sm" type="number" placeholder="e.g. 45000" value={quoteForm.estimatedPrice} onChange={e => { setQuoteForm(f => ({ ...f, estimatedPrice: e.target.value })); setQuoteSaved(false); }} />
                    </div>
                    <div>
                      <label className="text-xs font-600 block mb-1">Price per KG (¥)</label>
                      <input className="input-field text-sm" type="number" placeholder="e.g. 28" value={quoteForm.pricePerKg} onChange={e => { setQuoteForm(f => ({ ...f, pricePerKg: e.target.value })); setQuoteSaved(false); }} />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-600 block mb-1">ETA (date) <span className="text-red-500">*</span></label>
                    <input className="input-field text-sm" type="date" value={quoteForm.eta} onChange={e => { setQuoteForm(f => ({ ...f, eta: e.target.value })); setQuoteSaved(false); }} />
                  </div>
                  <div>
                    <label className="text-xs font-600 block mb-1">Note / Remarks</label>
                    <textarea className="input-field text-sm resize-none" rows={3} placeholder="Any additional notes for the client…" value={quoteForm.note} onChange={e => { setQuoteForm(f => ({ ...f, note: e.target.value })); setQuoteSaved(false); }} />
                  </div>
                </div>
                <button onClick={saveQuote} className="mt-4 w-full py-2.5 text-sm font-600 rounded-lg bg-[#4A3B52] text-white hover:bg-[#1A1423] transition-colors inline-flex items-center justify-center gap-2">
                  ...
                  <div>
                    ...
                    {warehouseSlip && (
                      warehouseSlip.startsWith('data:image')
                        ? <img src={warehouseSlip} alt="Warehouse slip" className="max-h-64 rounded-lg border border-border object-contain" />
                        : <div className="flex items-center gap-2 bg-muted/40 rounded-lg px-3 py-2 w-fit">
                            <span className="text-xl">📄</span>
                            <a href={warehouseSlip} download className="text-sm font-600 text-[#4A3B52] hover:underline">View PDF</a>
                          </div>
                    )}
                  </div>
                )}
                  ...
                  {cargoConfirmation ? (
                    <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                      ...
                      <div className="space-y-1.5 text-sm">
                        ...
                        {cargoConfirmation.notes && <p><span className="font-600">Notes:</span> <span className="text-muted-foreground">{cargoConfirmation.notes}</span></p>}
                      </div>
                    </div>
                  ) : (
                    <div>
                      <p className="text-sm text-muted-foreground mb-4">Confirm once you have physically received the cargo at the warehouse</p>
                      <div className="space-y-3">
                        <div>
              ...
              {chatMessages.map(msg => (
                <div key={msg.id} className={`flex gap-3 ${msg.sender === 'admin' ? 'flex-row-reverse' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-700 flex-shrink-0 ${msg.sender === 'admin' ? 'bg-[#5c5470]' : 'bg-[#c17b5c]'}`}>
                    {msg.sender === 'admin' ? 'ME' : clientInitials}
                  </div>
